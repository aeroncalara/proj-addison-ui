...
class Payslip_Page extends Component {
	...
	getEmployee = async () => {
		...
		`
			query
			{
				getEmployee(employee_id: "${this.props.match.id}")
				{
					_id
					person
					{
	...
	getIncentives = async () =>{
		...
		`
			query{
				getAllActiveIncentivesOfEmployee(employee_id:"${this.props.match.id}"){
					  date_incurred
					  description
					  amount
					is_active
	...
	getTimeLogs = async () =>{
		...
		`
			query{
				getAttendanceOfEmployee(employee_id:"${this.props.match.id}"){
					sessions{
						date
						time_in
						time_out
